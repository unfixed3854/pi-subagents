...
describe("subagent guidance", () => {
	it("builds a marked guidance block with parent workflow protocol and agent bullets", () => {
		...
		assert.match(block, /feature implementation or other runtime\/product\/user-visible implementation: no direct implementation/i);
		assert.match(block, /consult `oracle` when design tradeoffs exist/i);
		assert.match(block, /approval before implementation/i);
		assert.match(block, /use `worker` for scoped edits from an approved spec\/plan\/task path or explicit parent\/user workflow-skip approval, and `reviewer` for meaningful changes/i);
		...
		assert.match(block, /Direct action is only for read-only answers, formatting\/typo-only edits, or mechanical edits that do not change runtime\/product\/user-visible behavior/i);
		assert.match(block, /does not apply to feature implementation, bug fixes, test-failure fixes, unexpected behavior, async\/UI behavior, integration\/API behavior, completion claims, or other implementation that changes runtime\/product\/user-visible behavior/i);
		assert.match(block, /Before any `edit` or `write`, classify the request/i);
		assert.match(block, /feature implementation, bug fix, test-failure fix, integration\/API behavior, async\/UI behavior, or other implementation that changes runtime\/product\/user-visible behavior/i);
		assert.match(block, /Parent owns workflow-skip decisions/i);
		assert.match(block, /ask the user with `ask_user` before skipping the workflow/i);
		assert.match(block, /Direct action never overrides selected workflow skills/i);
		assert.match(block, /Parent routes, `worker` edits scoped tasks, and `reviewer` checks meaningful changes/i);
		assert.match(block, /approved spec\/plan\/task path or explicit parent\/user workflow-skip approval/i);
